...
1. `scrubbed_output.txt` — cleaned agent output passed to Validator
2. `conflict_log.json` — conflict records extracted from CONFLICTS section
   (path: /root/openclaw_phase6/validation/)

Note: scrubber_report.json was never implemented (T-06, resolved 2026-05-11).
Scrubber metrics (ids_seen, ids_kept, ids_removed, unsupported_groups,
uncited_claims_removed) are emitted to stdout and captured in the cron log only.
...
Per run, the scrubber emits these metrics to stdout (captured in cron log):
...
Note: These metrics are not written to a JSON file. scrubber_report.json
was never implemented (T-06, resolved 2026-05-11).
