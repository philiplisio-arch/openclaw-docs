...
## EXECUTION MODEL (FULL — PHASE 6.8)
...
9. invoke agent (with numbered VALID_SOURCES list — Phase 6.8)
10. agent produces agent_output.txt (citations in [source_numbers: N] format)
11. **Resolver** (resolve_source_numbers.py) maps [source_numbers: N] → [result_ids: res_xxx]; produces resolved output
12. **Scrubber** runs on resolved output → produces scrubbed_output.txt + conflict_log.json; metrics to stdout/cron log
13. **Control Layer** checks structural completeness of scrubbed_output.txt
14. **Validator** runs on scrubbed_output.txt + retrieval_package.json → produces validation_result.json (PASS/WARN/FAIL)
15. **Delivery Gate** applies decision matrix and delivers to Lark or blocks
