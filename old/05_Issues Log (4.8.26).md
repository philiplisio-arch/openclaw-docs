...
## Issue #23 — Orchestrator Not Connected to Production (RESOLVED)

Status: ✅ RESOLVED (2026-04-08)

Description:
Production system (cron/webhook) was still executing legacy prompt-driven pipeline despite Phase 5 orchestrator being built and validated.

Resolution:
- Replaced /root/run_light_to_lark.sh entry point
- Connected cron → orchestrator pipeline
- Removed legacy execution path from production
- Added wrapper to handle output formatting and relay delivery

Result:
Production system now fully runs:
Trigger → Orchestrator → Agent → Relay → Lark

---

## Issue #24 — Output Format Regression (PARTIALLY RESOLVED)

Status: 🟡 PARTIAL

Description:
Initial Phase 5 integration removed:
- timestamped header
- LinkedIn Draft
- regional signal sections

Resolution:
- Wrapper updated to reconstruct final output format
- Header restored
- Signal sections restored

Remaining:
- LinkedIn Draft not yet restored
- Minor formatting issues (publisher field, duplication)

---

## Issue #25 — Baidu Retrieval Incomplete

Status: 🟡 OPEN

Description:
Baidu executor currently runs in simulated/stub mode.

Impact:
- Retrieval package marked as partial
- Chinese source coverage incomplete

Next Step:
- Implement real Baidu API integration
- Validate retrieval contribution vs Brave

---

## Issue #26 — API Rate Limit (Moonshot/Kimi)

Status: 🟡 OBSERVED

Description:
Rapid repeated runs triggered provider rate limiting.

Impact:
- Temporary execution failures during testing

Resolution:
- No system change required
- Operational awareness: allow cooldown between runs

Future:
- Add retry/backoff logic in Phase 5.5
