...
## 🚧 PHASE 6.5–6.8 — SOFT LAYER

**Gate cleared** (6.1–6.4 all complete as of 2026-05-05)

### 🚧 6.5 — Conflict Handling Upgrade — ACTIVE

Implementation complete. Sole remaining exit criterion: non-zero conflict path
verified in synthetic test.

Exit criteria:
- ✔ Conflicting claims across sources explicitly surfaced — implementation complete
...
- ⏳ Non-zero conflict path exercised — synthetic test pending

### ⏳ 6.6 — Citation Substitution (pending Phase 6.5 close)

Scope decided 2026-05-06. Delivery script only — single layer, no architecture change.

Exit criteria:
- result_id tokens replaced with publisher/date strings in all delivered Lark output
- Substitution lookup grounded in retrieval_package.json — no invented publisher names
- Validator and scrubber logs continue to use result_ids unchanged
- Two consecutive successful deliveries with human-readable citations confirmed
- No delivery failures introduced

### 6.7 — Low-Signal Handling Refinement
*(previously 6.6)*
...
### 6.8 — Output Consistency Hardening
*(previously 6.7)*
